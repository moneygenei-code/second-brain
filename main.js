...
const { logToSecondBrain, getAllLogs, getAggregatedAnalytics } = require('./secondBrain');
...
ipcMain.handle('run-architect', async () => {
  try {
    ...
    if (process.env.GEMINI_API_KEY) {
      ...
      parsed = architectAgent.parseResponse(rawResponse);
    } else {
      // Fallback for missing API key in prototype
      console.warn('[Architect] GEMINI_API_KEY missing. Using mock response.');
      parsed = architectAgent.parseResponse(JSON.stringify({
        agent: "Architect",
        headline: "Architectural Analysis & Future Plan",
        summary: "System performance is stable. (MOCK - API Key Missing)",
        insights: [
          "Deals are successful when initiated by specialist agents.",
          "Logging latency is minimal."
        ],
        future_plan: [
          "Step 1: Implement real-time dashboard (Complete)",
